...
function cardLimit() { return Number.isFinite(deck?.maxCards) ? Math.min(deck.maxCards, MAX_CARDS) : MAX_CARDS; }
function controls() { const empty = !cards.length, limit = cardLimit(), full = cards.length >= limit; reveal.disabled = empty; previous.disabled = empty || index === 0; next.disabled = empty || index === cards.length - 1; edit.disabled = empty; remove.disabled = empty; add.disabled = full; add.textContent = full ? 'Card limit reached' : '+ Add a card'; $('#deck-card-count').textContent = `${cards.length} of ${limit} cards in deck`; }
function renderCard() {
  hideAnswer();
...
function openAdd() { if (!deck || add.disabled) return; addForm.reset(); $('#add-card-message').textContent = ''; $('#deck-capacity-message').textContent = `Complete every field. This deck can contain up to ${cardLimit()} cards.`; addDialog.showModal(); addForm.elements.word.focus(); }
function saveAdd(event) { event.preventDefault(); const values = formValues(addForm); if (Object.values(values).some((value) => !value)) { $('#add-card-message').textContent = 'Please complete every field.'; return; } if (cards.length >= cardLimit()) return; try { const savedCard = addCard(deckId, { ...values, type: values.type.toUpperCase() }); if (!savedCard) { $('#add-card-message').textContent = 'A card with this word and reading already exists in this deck.'; return; } refresh(); index = cards.length - 1; close(addDialog); renderCard(); } catch (error) { $('#add-card-message').textContent = error?.message || 'The card could not be saved.'; } }
