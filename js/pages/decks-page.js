...
if (!storage) {
  ...
  if (statusElement) {
    statusElement.textContent =
      'Recall could not initialize browser storage. Reload the page or check browser storage permissions.';
  }
  return;
}
...
function announceStorageStatus() {
  const current = getStorageStatus();
  if (current?.message) announce(current.message);
}

function deckMetadata(formData) {
  return Object.fromEntries(
    ['title', 'description', 'language', 'level'].map((key) => [key, String(formData.get(key)).trim()])
  );
}
...
function renderDecks() {
  ...
  decks.forEach((deck) => {
    ...
    meta.textContent = `${deck.language} · ${deck.level}`;
    ...
    description.textContent = deck.description || 'No description yet.';
    ...
    study.className = 'button study-deck-link';
    const importedCardId = importedCardTargets.get(deck.id);
    study.href = `study.html?deck=${encodeURIComponent(deck.id)}${importedCardId ? `&card=${encodeURIComponent(importedCardId)}` : ''}`;
    study.textContent = 'Study';

    actions.append(
      study,
      actionButton('Export', 'export-deck'),
      actionButton('Edit', 'edit-deck'),
      actionButton('Delete', 'delete-deck')
    );
    article.append(meta, title, description, count, actions);
    grid.append(article);
  });
}
...
function openCreateDialog() {
  ...
  formTitle.textContent = 'Create deck';
  dialog.showModal();
...
function openEditDialog(id) {
  ...
  formMessage.textContent = '';
  ['title', 'description', 'language', 'level'].forEach((field) => {
    form.elements[field].value = deck[field];
  });
  dialog.showModal();
  form.elements.title.focus();
}
...
function saveDeck(event) {
  ...
  try {
    if (editingDeckId) {
      ...
      if (!updatedDeck) {
        formMessage.textContent = 'A deck with this title already exists.';
        return;
      }
      closeDeckDialog();
      renderDecks();
    ...
    formMessage.textContent = error?.message || 'The deck could not be saved.';
  }
}
...
function openDeleteDialog(id) {
  ...
  if (!deck) return;
  deletingDeckId = id;
  deleteDescription.textContent = `“${deck.title}” and its ${deck.cards.length} ${deck.cards.length === 1 ? 'card' : 'cards'} will be removed from this browser. This cannot be undone.`;
...
function exportDestinationMode() {
  ...
  try {
    mobileShare = Boolean(
      ...
      navigator.share &&
      navigator.canShare?.({ files: [new File([''], 'recall.txt', { type: 'text/plain' })] })
    );
  } catch {
    mobileShare = false;
  }
...
function openExportDialog(id) {
  ...
  exportDescription.textContent =
    mode === 'share'
      ? 'Choose a format, then select a destination from your device’s share menu.'
      : mode === 'save'
        ? 'Choose a format, then select where to save the file.'
        : 'Choose a format to download the file. Your browser’s download settings control whether you are asked where to save it.';
  exportSubmit.textContent =
    mode === 'share' ? 'Choose destination' : mode === 'save' ? 'Choose save location' : 'Download file';
...
async function submitExport(event) {
  ...
  if (!deck) {
    exportMessage.textContent = 'This deck is no longer available.';
    return;
  }
  const submit = exportForm.querySelector('[type="submit"]');
  submit.disabled = true;
  exportMessage.textContent = 'Preparing your file…';
  try {
    ...
    announce(
      result === 'shared'
        ? 'Deck sent to your chosen destination.'
        : result === 'saved'
          ? 'Deck saved to your chosen location.'
          : 'Deck download started. Your browser controls the download location.'
    );
  } catch (error) {
...
async function handleImport(event) {
  ...
  try {
    ...
    announce(`Import blocked: ${error?.message || 'the file is invalid.'}`);
  } finally {
    fileInput.value = '';
  }
}
...
function renderImportCards(cards) {
  ...
  visibleCards.forEach((item) => {
    const { card } = item;
    const row = document.createElement(item.isDuplicate ? 'div' : 'label');
    row.className = `import-card-option${item.isDuplicate ? ' import-card-option-duplicate' : ''}`;
    ...
    appendImportField(fields, 'Reading', card.reading);
    appendImportField(fields, 'Type', card.type);
    appendImportField(fields, 'Meaning', card.meaning);
    appendImportField(fields, 'Example', card.example);
    appendImportField(fields, 'Translation', card.translation);
    content.append(word);

    if (item.isDuplicate) {
      const duplicate = document.createElement('span');
      duplicate.className = 'import-duplicate-status';
      duplicate.textContent = 'Duplicate — matches an existing card';

      const actionLabel = document.createElement('label');
      actionLabel.className = 'import-duplicate-action';
      actionLabel.textContent = 'Import action';

      const select = document.createElement('select');
      select.className = 'import-duplicate-action-select';
      select.dataset.cardId = card.id;
      select.setAttribute('aria-label', `Import action for duplicate card ${card.word}`);
      [
        ['skip', 'Skip'],
        ['add-copy', 'Add copy'],
        ['replace', 'Replace existing card']
      ].forEach(([value, text]) => {
      ...
      content.append(duplicate, actionLabel);
      row.append(document.createElement('span'), content);
    } else {
      const checkbox = document.createElement('input');
      checkbox.type = 'checkbox';
      checkbox.className = 'import-card-checkbox';
      checkbox.dataset.cardId = card.id;
      checkbox.checked = pendingImport.selectedCardIds.has(card.id);
  ...
  if (visibleCards.length === 0) {
    ...
    empty.className = 'import-filter-empty';
    empty.textContent =
      pendingImport.filter === 'duplicates'
        ? 'No duplicate cards were found in this import.'
        : pendingImport.filter === 'nonduplicates'
          ? 'No nonduplicate cards were found in this import.'
          : 'No cards are available in this import.';
...
function renderExistingDeckOptions() {
  ...
  getAllDecks().forEach((deck) => {
    ...
    option.textContent = `${deck.title} (${deck.cards.length} cards · ${available} spaces available)`;
...
function selectedDestinationMode() {
  return importForm.elements['import-destination'].value;
}
...
function updateImportDuplicateMetadata() {
  ...
  pendingImport.cards.forEach((item) => {
    ...
    nextActions.set(
      item.card.id,
      previous?.matchingCardId === item.matchingCardId
        ? previous
        : { action: 'skip', matchingCardId: item.matchingCardId }
    );
  });
...
function updateImportControls() {
  if (!pendingImport) return;
  const chosenDuplicates = [...pendingImport.duplicateActions.values()].filter(
  ...
  importName.disabled = !creating;
  importName.required = creating;
  existingDeckSelect.disabled = creating;
  existingDeckSelect.required = !creating;
...
function setAllNewCardSelection(cards, selectedCardIds, selected) {
  ...
  if (selected) {
    cards.filter((item) => !item.isDuplicate).forEach((item) => {
...
function setAllImportCards(selected) {
  ...
  importCardList.querySelectorAll('.import-card-checkbox').forEach((checkbox) => {
...
function cancelImportReview() {
  pendingImport = null;
  importForm.reset();
  importCardList.replaceChildren();
  importMessage.textContent = '';
...
function confirmImport(event) {
  event.preventDefault();
  if (!pendingImport) return;
  const selectedCards = pendingImport.deck.cards.filter(
    (card) => pendingImport.selectedCardIds.has(card.id) || pendingImport.duplicateActions.has(card.id)
  );
  const duplicateActions = [...pendingImport.duplicateActions].map(([cardId, decision]) => ({ cardId, ...decision }));
  const mode = selectedDestinationMode();
  if (selectedCards.length === 0) {
    importMessage.textContent = 'Select at least one card to import.';
    updateImportControls();
    return;
  }
  ...
  try {
    ...
    announce(
      result.created
        ? `${count} ${count === 1 ? 'card' : 'cards'} imported into new deck “${title}”.`
        : `Import complete for “${title}”: ${result.nonduplicateAdded} new, ${result.copiesAdded} copied, ${result.replacedCount} replaced, ${result.skippedCount} skipped.`
    );
  } catch (error) {
    importMessage.textContent = error?.message || 'The selected cards could not be imported.';
    updateImportControls();
  }
}
...
document.querySelector('#deselect-all-import-cards').addEventListener('click', () => setAllImportCards(false));
document.querySelector('#close-import-review').addEventListener('click', cancelImportReview);
document.querySelector('#cancel-import-review').addEventListener('click', cancelImportReview);
importForm.addEventListener('submit', confirmImport);

importCardList.addEventListener('change', (event) => {
  if (!pendingImport || !event.target.matches('.import-card-checkbox')) return;
  const cardId = event.target.dataset.cardId;
  if (!pendingImport.deck.cards.some((card) => card.id === cardId)) return;
  if (event.target.checked) pendingImport.selectedCardIds.add(cardId);
  else pendingImport.selectedCardIds.delete(cardId);
  importMessage.textContent = '';
  updateImportControls();
});
importCardList.addEventListener('change', (event) => {
  if (!pendingImport || !event.target.matches('.import-duplicate-action-select')) return;
  const decision = pendingImport.duplicateActions.get(event.target.dataset.cardId);
  if (!decision) return;
  pendingImport.duplicateActions.set(event.target.dataset.cardId, { ...decision, action: event.target.value });
  importMessage.textContent = '';
  updateImportControls();
});
importForm.addEventListener('change', (event) => {
  if (!pendingImport || event.target.name !== 'import-filter') return;
  pendingImport.filter = event.target.value;
  renderImportCards(pendingImport.cards);
});
importForm.addEventListener('change', (event) => {
  if (event.target.name === 'import-destination') {
    importMessage.textContent = '';
    updateImportDuplicateMetadata();
    updateImportControls();
  }
});
importName.addEventListener('input', updateImportControls);
existingDeckSelect.addEventListener('change', () => {
  updateImportDuplicateMetadata();
  updateImportControls();
});

document.querySelector('#close-deck-dialog').addEventListener('click', closeDeckDialog);
document.querySelector('#cancel-deck-dialog').addEventListener('click', closeDeckDialog);
form.addEventListener('submit', saveDeck);

grid.addEventListener('click', (event) => {
  const tile = event.target.closest('.deck-tile');
  if (!tile) return;
  if (event.target.closest('.export-deck')) openExportDialog(tile.dataset.deckId);
  if (event.target.closest('.edit-deck')) openEditDialog(tile.dataset.deckId);
  if (event.target.closest('.delete-deck')) openDeleteDialog(tile.dataset.deckId);
});

document.querySelector('#close-delete-deck').addEventListener('click', closeDeleteDialog);
document.querySelector('#cancel-delete-deck').addEventListener('click', closeDeleteDialog);
document.querySelector('#delete-deck-form').addEventListener('submit', confirmDelete);
...
importDialog.addEventListener('click', (event) => {
  if (event.target === importDialog) cancelImportReview();
});
...
importDialog.addEventListener('close', () => {
  if (pendingImport) cancelImportReview();
});
...
if (location.protocol === 'file:') {
  announce('Recall must be opened through a local web server. Direct file mode cannot share deck storage with the Study page.');
} else {
  announceStorageStatus();
}
})();
